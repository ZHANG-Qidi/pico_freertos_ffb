...
void _startADC3PinConversionLowSide() { instance.startADCScan(); }
/**
 *  function reading an ADC value and returning the read voltage
 *
...
float _readADCVoltageLowSide(const int pinA) {
    ...
    if (pinA == _pinA) return instance.toVolts(a);
    if (pinA == _pinB) return instance.toVolts(b);
    if (pinA == _pinC) return instance.toVolts(c);
...
SAMDCurrentSenseADCDMA *SAMDCurrentSenseADCDMA::getHardwareAPIInstance() { return &instance; }
...
bool SAMDCurrentSenseADCDMA::readResults(uint16_t &a, uint16_t &b, uint16_t &c) {
    if (ADC->CTRLA.bit.ENABLE) return false;
...
float SAMDCurrentSenseADCDMA::toVolts(uint16_t counts) { return counts * countsToVolts; }
...
void SAMDCurrentSenseADCDMA::initADC() {
    ...
    */
    ...
    ;  // sample length in 1/2 CLK_ADC cycles, see GCLK_ADC and ADC_CTRLB_PRESCALER_DIV16
